function runSearchlight_subspaceGener(Searchlight,inFiles,outFiles,rsaFunction,run,tempFilesDir,varargin)
...
%   Alon's addition - infiles are the preprocessed data file, i.e.
%   length(infiles)==(number of TRs)
...
while (~isDone)
    ...
    save(sprintf(fullfile(tempFilesDir,[run '_temp_%2.2d.mat']),b),'T');
    isCalc(j) = true;
    isDone    = all(isCalc);
    b=b+1;
end
...
for b=1:numBlocks;
    load(sprintf(fullfile(tempFilesDir,[run '_temp_%2.2d.mat']),b));
...
delete(fullfile(tempFilesDir,[run '_temp*.mat']));
